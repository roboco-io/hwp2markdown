...
HWP(Hangul Word Processor)는 한컴오피스의 워드프로세서 파일 포맷으로, 버전마다 호환되지 않는 포맷을 가진 것으로 악명이 높다. 본 문서는 hwp2markdown 도구 개발을 위한 HWP 포맷 조사 결과를 정리한다.
